...
class Cita(db.Model):
    ...
    seguimientos                   = db.relationship('Seguimiento', backref='cita', lazy=True,
                                                     cascade='all, delete-orphan')


class Seguimiento(db.Model):
    """Notas de seguimiento que el doctor agrega a una cita/paciente."""
    id         = db.Column(db.Integer,    primary_key=True)
...
def dashboard():
    try:
        if current_user.role == 'admin':
            ...
            # Citas sin asignar + citas propias de este doctor
            from sqlalchemy import or_
            citas = Cita.query.filter(
                or_(Cita.doctor_id == None, Cita.doctor_id == current_user.id)
            ).order_by(Cita.fecha).all()
            pacientes = {u.id: u.username for u in User.query.filter_by(role='paciente').all()}
            # Mis pacientes: pacientes únicos de citas completadas/en_proceso asignadas a mí
            mis_ids = {c.paciente_id for c in Cita.query.filter_by(doctor_id=current_user.id).all()}
            mis_pacientes = User.query.filter(User.id.in_(mis_ids)).all() if mis_ids else []
...
@app.route('/tomar_cita/<int:cita_id>', methods=['POST'])
@login_required
def tomar_cita(cita_id):
    """Doctor toma una cita sin asignar — la marca como suya."""
    try:
        if current_user.role != 'doctor':
            flash('Acceso no autorizado.')
            return redirect(url_for('dashboard'))
        ...
        if not cita:
            flash('Cita no encontrada.')
            return redirect(url_for('dashboard'))
        if cita.doctor_id and cita.doctor_id != current_user.id:
            flash('Esta cita ya fue tomada por otro médico.')
            return redirect(url_for('dashboard'))
        cita.doctor_id = current_user.id
        if cita.status == 'pendiente':
            cita.status = 'en_proceso'
        db.session.commit()
        logger.info('CITA TOMADA | doctor="%s" cita_id=%s', current_user.username, cita_id)
        flash('Cita asignada a tu panel.')
    except Exception:
        db.session.rollback()
        logger.exception('ERROR al tomar cita_id=%s', cita_id)
        flash('Error al tomar la cita.')
    return redirect(url_for('dashboard'))
...
def ver_paciente(paciente_id):
    """Panel de seguimiento completo de un paciente — solo el doctor asignado."""
    try:
        if current_user.role != 'doctor':
            flash('Acceso no autorizado.')
            return redirect(url_for('dashboard'))
        ...
        if not paciente:
            ...
            return redirect(url_for('dashboard'))
        # Solo el doctor que atendió al menos una cita de este paciente
        tiene_relacion = Cita.query.filter_by(
            paciente_id=paciente_id, doctor_id=current_user.id).first()
        if not tiene_relacion:
            flash('No tienes acceso al historial de este paciente.')
            return redirect(url_for('dashboard'))
        citas = Cita.query.filter_by(
            paciente_id=paciente_id, doctor_id=current_user.id
        ).order_by(Cita.fecha.desc()).all()
        return render_template('paciente_detalle.html', paciente=paciente, citas=citas)
    except Exception:
        logger.exception('ERROR en ver_paciente id=%s', paciente_id)
        flash('Error al cargar el paciente.')
        return redirect(url_for('dashboard'))


@app.route('/seguimiento/<int:cita_id>', methods=['POST'])
@login_required
def agregar_seguimiento(cita_id):
    """Agrega una nota de seguimiento a una cita."""
    try:
        if current_user.role != 'doctor':
            flash('Acceso no autorizado.')
            return redirect(url_for('dashboard'))
        cita = Cita.query.get(cita_id)
        if not cita or cita.doctor_id != current_user.id:
            flash('Cita no encontrada o no autorizada.')
            return redirect(url_for('dashboard'))
        nota = sanitizar(request.form.get('nota', ''), max_len=1000)
        if not nota:
            flash('La nota no puede estar vacía.')
            return redirect(url_for('ver_paciente', paciente_id=cita.paciente_id))
        s = Seguimiento(
            cita_id=cita_id,
        ...
        db.session.add(s)
        db.session.commit()
        logger.info('SEGUIMIENTO | doctor="%s" cita_id=%s', current_user.username, cita_id)
        flash('Nota de seguimiento guardada.')
    except Exception:
        db.session.rollback()
        logger.exception('ERROR en seguimiento cita_id=%s', cita_id)
        flash('Error al guardar la nota.')
    return redirect(url_for('ver_paciente', paciente_id=cita.paciente_id))
...
def migrar_bd():
    ...
    # Migrar tabla seguimiento si no existe
    try:
        inspector2 = inspect(db.engine)
        tablas = inspector2.get_table_names()
